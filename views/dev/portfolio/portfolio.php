<?php
use App\Path;

$title = "Portfolio";
$path = new Path();
$path->getStyle("../css/dev/portfolio_dev.css");
?>
  
<!--////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////// PORTFOLIO EN GRILLE ////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////-->

<div class="container">

  <h1 class="text-center my-4">Portfolio en grille</h1>
  <div class="portfolio" id="js-portfolio">
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Nom du projet</h2>
      <div class="project__description">Descriptif du projet</div>
      <div class="project__body js-body">
        <h2>Nom du projet 1 (déroulé)</h2>
        <p>
          Descriptif du projet (déroulé) sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Nom du projet</h2>
      <div class="project__description">Consectetur adipisicing elit</div>
      <div class="project__body js-body">
        <h2>Nom du projet 2 (déroulé)</h2>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Lorem ipsum</h2>
      <div class="project__description">Consectetur adipisicing elit</div>
      <div class="project__body js-body">
        <h2>Lorem ipsum dolor sit amet</h2>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Lorem ipsum</h2>
      <div class="project__description">Consectetur adipisicing elit</div>
      <div class="project__body js-body">
        <h2>Lorem ipsum dolor sit amet</h2>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Lorem ipsum</h2>
      <div class="project__description">Consectetur adipisicing elit</div>
      <div class="project__body js-body">
        <h2>Lorem ipsum dolor sit amet</h2>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Lorem ipsum</h2>
      <div class="project__description">Consectetur adipisicing elit</div>
      <div class="project__body js-body">
        <h2>Lorem ipsum dolor sit amet</h2>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Lorem ipsum</h2>
      <div class="project__description">Consectetur adipisicing elit</div>
      <div class="project__body js-body">
        <h2>Lorem ipsum dolor sit amet</h2>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Lorem ipsum</h2>
      <div class="project__description">Consectetur adipisicing elit</div>
      <div class="project__body js-body">
        <h2>Lorem ipsum dolor sit amet</h2>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Lorem ipsum</h2>
      <div class="project__description">Consectetur adipisicing elit</div>
      <div class="project__body js-body">
        <h2>Lorem ipsum dolor sit amet</h2>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Lorem ipsum</h2>
      <div class="project__description">Consectetur adipisicing elit</div>
      <div class="project__body js-body">
        <h2>Lorem ipsum dolor sit amet</h2>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Lorem ipsum</h2>
      <div class="project__description">Consectetur adipisicing elit</div>
      <div class="project__body js-body">
        <h2>Lorem ipsum dolor sit amet</h2>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Lorem ipsum</h2>
      <div class="project__description">Consectetur adipisicing elit</div>
      <div class="project__body js-body">
        <h2>Lorem ipsum dolor sit amet</h2>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Lorem ipsum</h2>
      <div class="project__description">Consectetur adipisicing elit</div>
      <div class="project__body js-body">
        <h2>Lorem ipsum dolor sit amet</h2>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
    <div class="project js-item" tabindex="0">
      <img src="http://via.placeholder.com/350x350" alt="" class="project__image">
      <h2 class="project__name">Lorem ipsum</h2>
      <div class="project__description">Consectetur adipisicing elit</div>
      <div class="project__body js-body">
        <h2>Lorem ipsum dolor sit amet</h2>
        <p>
          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium consequatur, consequuntur, dicta eius
          facilis ipsa ipsum itaque magnam molestias mollitia nemo nihil provident quod saepe sed temporibus ut veniam
          voluptates!
        </p>
        <p>
          Ab architecto, atque autem blanditiis culpa distinctio ea eius harum, hic laudantium maiores possimus, quia
          repellat sunt voluptatum? Aperiam doloremque dolorum facilis ipsum iusto laborum maiores nisi odit officia
          repellendus.
        </p>
        <p>
          <a href="#">En savoir plus</a>
        </p>
      </div>
    </div>
  </div> <!-- Fin de div class = portfolio" id= js-portfolio -->
</div>

<?php
    $path->getJs("../js/dev/portfolio/app.js");
    $path->getJs("../js/dev/portfolio/polyfill_portfolio.js");
?>

